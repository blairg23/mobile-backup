...
def resolve_adb(cfg):
    ...
    if Path(default).exists():
        return default
    print("ERROR: adb not found. Set 'adb_path' in config.yaml.", file=sys.stderr)
    sys.exit(2)
...
def ensure_device(adb_exec, adb_device):
    ...
    if not lines:
        print("No devices visible to adb.")
        sys.exit(2)
    if "unauthorized" in lines[0]:
        print("Device unauthorized. Revoke & re-approve USB debugging.")
        sys.exit(2)
...
def adb_list(adb_exec, adb_device, path: str, include_hidden: bool):
    ...
    out = subprocess.run(
        [adb_exec]
        + (["-s", adb_device] if adb_device else [])
...
def main():
    ...
    ap.add_argument(
        ...
        nargs="?",
        default="/sdcard",
        help="device path to list (default: /sdcard)",
    )
    ap.add_argument("-a", "--all", action="store_true", help="include hidden entries")
    ap.add_argument(
        "-n", "--limit", type=int, default=10, help="how many names to show"
    )
    args = ap.parse_args()
